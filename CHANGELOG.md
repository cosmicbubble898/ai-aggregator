...
- **Landing page + demo chat** — replaced the plain placeholder with a designed landing page built on the tokens (nav, hero with a generated product-preview image, feature cards, how-it-works steps, an interactive client-side demo chat, footer); Google sign-in preserved.
- **BYOK key management (P0-2)** — `/settings` page to add/replace/remove **OpenRouter** + **AtlasCloud** keys: validated with a live provider call, **encrypted at rest** (AES-256-GCM, master key in `ENCRYPTION_KEY`), never returned to the browser (only a last-4 hint). New `provider_key` table (one row per user+provider); server-only crypto + key helpers; `session.user.id` exposed for per-user scoping. Specced in `docs/specs/byok.md` + **ADR-0010**.
